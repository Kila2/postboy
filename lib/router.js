...
import db from './db';
import SingletonBean from './SingletonBean';
...
const router = (config) => {
  ...
  //   let { isLogin } = SingletonBean.isLogin;
  //   if (isLogin) {
  ...
  appRouter.use(bodyParser.json({ limit: '1mb' }));
  ...
  appRouter.use(methodOverride((req) => {
  ...
  const addTrailingSlash = s => s + (s[s.length - 1] === '/' ? '' : '/');
  const buildBaseHref = (originalUrl, reqUrl) => {
    if (reqUrl === '/') {
  ...
  appRouter.all('*', (req, res, next) => {
    ...
    mongo.updateDatabases(mongo.adminDb, (databases) => {
  ...
  appRouter.all('*', (req, res, next) => {
    ...
    query.equalTo('token', req.header.token);
  ...
  appRouter.param('database', (req, res, next, id) => {
  ...
  appRouter.param('collection', (req, res, next, id) => {
    ...
    mongo.connections[req.dbName].collection(id, (err, coll) => {
      if (err || coll === null) {
        req.session.error = 'Collection not found!';
        return res.redirect(`${res.locals.baseHref}db/${req.dbName}`);
      }
  ...
  appRouter.param('document', (req, res, next, id) => {
    ...
    if (obj_id) {
      ...
      req.collection.findOne({ _id: obj_id }, (err, doc) => {
        if (err) {
        ...
        if (doc === null) {
          ...
          req.collection.findOne({ _id: id }, (err, doc) => {
            if (err) {
              req.session.error = `Error: ${err}`;
              return res.redirect(`${res.locals.baseHref}db/${req.dbName}/${req.collectionName}`);
            }
  ...
  appRouter.param('prop', (req, res, next, prop) => {
    req.prop = req.document[prop];
    next();
  });

  // GridFS (db)/gridFS/(bucket)
  appRouter.param('bucket', (req, res, next, id) => {

    req.bucketName = id;
    res.locals.bucketName = id;

    mongo.connections[req.dbName].collection(`${id}.files`, (err, filesConn) => {
      ...
      filesConn.find({}).toArray((err, files) => {
        if (err || files === null) {
  ...
  // GridFS files
  appRouter.param('file', (req, res, next, id) => {
  ...
  //login
  appRouter.post('/login', mongoMiddleware, configuredRoutes.login);
